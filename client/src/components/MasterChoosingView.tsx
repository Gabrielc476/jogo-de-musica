      ...
      <div className="flex flex-col flex-1 gap-3 py-1 animate-in fade-in">
        ...
        >
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl bg-white hover:bg-zinc-200 text-black font-bold text-xs shrink-0 transition-all active:scale-95 disabled:opacity-80 flex items-center justify-center gap-1.5 min-w-[84px]"
          >
            {isSearching ? (
              <>
                <svg
                ...
                <span className="text-[11px] font-extrabold">Buscando</span>
              </>
            ) : (
              'Buscar'
            )}
        ...
        {/* Lista de Resultados ou Animação de Carregamento */}
        <div className="flex flex-col gap-1.5 overflow-y-auto no-scrollbar flex-1 pr-0.5">
          {isSearching ? (
            <div className="flex flex-col gap-2 py-1 animate-in fade-in">
              {/* Header de status da busca */}
              <div className="flex items-center justify-between px-1">
                <span className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1.5">
                  ...
                  Buscando no YouTube e Catálogo...
                </span>
                <span className="text-[10px] text-zinc-500 font-mono">até 10s</span>
              </div>

              {/* Banner estilizado com equalizador em onda */}
              <div className="p-3 rounded-xl bg-gradient-to-r from-emerald-500/10 via-zinc-800/50 to-emerald-500/10 border border-emerald-500/20 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="flex items-end gap-1 h-4">
                    <span className="w-1 bg-emerald-400 rounded-full bar-wave" style={{ animationDelay: '0.1s' }} />
                    <span className="w-1 bg-emerald-400 rounded-full bar-wave" style={{ animationDelay: '0.4s' }} />
                    <span className="w-1 bg-emerald-400 rounded-full bar-wave" style={{ animationDelay: '0.2s' }} />
                    <span className="w-1 bg-emerald-400 rounded-full bar-wave" style={{ animationDelay: '0.5s' }} />
                    <span className="w-1 bg-emerald-400 rounded-full bar-wave" style={{ animationDelay: '0.3s' }} />
                  </div>
                  <p className="text-xs text-zinc-200 font-medium">Sintonizando frequências musicais...</p>
                </div>
              </div>

              {/* Skeleton Cards pulsantes de faixas */}
              {[1, 2, 3, 4, 5].map((item) => (
                <div
                  key={item}
                  className="p-2.5 rounded-xl bg-[#181818]/70 border border-white/5 flex items-center gap-3 animate-pulse"
                >
                  <div className="w-11 h-11 rounded-lg bg-zinc-800/80 shrink-0 flex items-center justify-center border border-white/5">
                    <div className="w-4 h-4 rounded-full border-2 border-zinc-600 border-t-emerald-400 animate-spin" />
                  </div>
                  <div className="flex-1 min-w-0 space-y-2">
                    <div
                      className="h-3 bg-zinc-700/60 rounded"
                      style={{ width: `${Math.max(45, 80 - item * 7)}%` }}
                    />
                    <div
                      className="h-2.5 bg-zinc-800/80 rounded"
                      style={{ width: `${Math.max(30, 50 - item * 6)}%` }}
                    />
                  </div>
                  <div className="h-3 w-8 bg-zinc-800/50 rounded shrink-0" />
                </div>
              ))}
            </div>
          ) : (
